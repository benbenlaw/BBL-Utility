...
public class SummoningBlockScreen extends AbstractContainerScreen<SummoningBlockMenu> {
    private static final Identifier TEXTURE = Utility.identifier("textures/gui/summoning_block_gui.png");
    private static final Identifier PROGRESS_ARROW = Core.identifier("progress_arrow");

    public SummoningBlockScreen(SummoningBlockMenu menu, Inventory inventory, Component title) {
    ...
    protected void renderBg(GuiGraphics guiGraphics, float v, int i, int i1) {
        ...
        if (menu.isCrafting()) {
            guiGraphics.blitSprite(RenderPipelines.GUI_TEXTURED, PROGRESS_ARROW, 24, 16, 0, 0, x + 76, y + 25, menu.getScaledProgress() + 1, 16);
        }
    }
